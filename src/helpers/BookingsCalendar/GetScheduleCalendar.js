...
export default class Calendar extends React.Component {
  ...
  onReshDate=(e,d)=>{
    ...
    this.callPropsFunc(d2)
    console.log(d1.length,d2.length,"kkkkkkkkkk")
    
  }

  callPropsFunc(resch_date){
    if(this.state.toberesch_date&&resch_date.length>0){
      console.log("resch_date",resch_date)
      this.props.changeData&&this.props.changeData(0,this.state.toberesch_date,resch_date)
    }
    else{
      this.props.changeData&&this.props.changeData(0,0,0)
    }
  }
  ...
  render() {
    ...
    let end_date=this.state.rangeSelect[this.state.rangeSelect.length-1];

    console.log(end_date, "rangeSelect")
      ...
      <div className="tail-datetime-calendar">
        ...
        {this.state.showDateTable && (
          <div className="calendar-date">
            ...
            {this.state.from_to_date.length>0?<label>{(this.state.from_to_date[0] +"to"+ end_date.substr(11).replace(/_/g, "-"))}</label>:<label>Select the Start Date</label>}
            </div>}
