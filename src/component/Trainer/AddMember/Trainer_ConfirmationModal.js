...
export default function ConfirmationModal(props) {
  ...
  const [Resch_Dates,setReschDates]=useState(props.Resch_Dates && props.Resch_Dates)
  // elipse function
  const ElipseOpen = () => {
  ...
  return (
    <div className="booking_confirm">
      ...
      <Grid container spacing={4} className="book_confirm_container">
        ...
        {props.Origin == "spec_reschedule" &&
          <Grid item xs={12} md={12} style={{ textAlign: "center" }}>
            <Link to={{ pathname: "/paymentmethod", state: { Trainer, ReschInfo,Resch_Dates, key: "Spec_Reschedule" } }}><Button className="confirm_b_btn">Confirm</Button></Link>
          </Grid>}
      </Grid>
    </div>
  )
