...
function BookingReSchedule(props) {
  ...
  const [reshc_dates,setReshcDates]=useState([])
  ...
  const getDates = (datearr,d1,d2) => {
    ...
    if(d1.length>0&&d2.length>0){
      ...
      dispatch(GetSlotsTrainerBooking(Slots))
      slots["FromDate"]=moment(from_date).format("YYYY-MM-DD");
      slots["ToDate"]=moment(to_date).format("YYYY-MM-DD");
      setReshcDates({toberesh:d1,rech_date:d2})
    }
    else{setList([])}
  }
  useEffect(() => {
 ...
 console.log(secheduleDates,reshc_dates,trainer, "locccc")
  ...
  return (
    <div className="booking_shedule_tra">
      ...
      >
        ...
        {session == 1 &&
         <ConfirmationModal ModalCloseClick={ModalCloseClick} Params={trainer} Resch_Info={slots} Resch_Dates={reshc_dates} Origin="spec_reschedule"/>
        }
      </Modal>
    </div>
  )
