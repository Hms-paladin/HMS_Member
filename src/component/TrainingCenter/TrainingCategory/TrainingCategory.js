...
import { getTrainerListForCenter } from "../../../actions/trainerCenterAction"
...
function TrainingCategory(props) {
  ...
  useEffect(() => {
    dispatch(getTrainerListForCenter())
  }, [])

  useEffect(() => {
    console.log(props.getTrainerCategoryList, "CategoryList")
    setList(props.getTrainerCategoryList)
  }, [props.getTrainerCategoryList])
        ...
        {List && List.map((data) => {
          return (
            <div className="img_cat_Container">
              <NavLink to="/feed"><img src={data.training_filename} /></NavLink>
              <div className="imgTitle">{data.trainingName}</div>
            </div>
          )
        })}
      </CarosalComp>
...
const mapStateToProps = (state) => ({
  getTrainerCategoryList: state.TrainerCenterReducer.getTrainerListForCenter || []
})
export default connect(mapStateToProps)(TrainingCategory)
