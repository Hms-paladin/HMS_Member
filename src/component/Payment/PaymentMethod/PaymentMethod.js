...
import {PatientTrainerBooking,PatientTrainerReschedule,SpecificDateReschedule} from '../../../actions/trainerdetailsaction'
...
function CancelPayment(props) {
  ...
  const payrReceivePush = () => {
    if(location.state&&location.state.key=="ClinicalLab"){
      dispatch(PatientLabBooking(params))
    }
    if(location.state&&location.state.key=="Trainer"){
      dispatch(PatientTrainerBooking(params))
    }
    if(location.state&&location.state.key=="Reschedule"){
      dispatch(PatientTrainerReschedule(params))
    }
    if(location.state&&location.state.key=="Spec_Reschedule"){
      dispatch(SpecificDateReschedule(params))
    }
  };
  ...
  useEffect(()=>{
    ...
    if(location.state&&location.state.key=="Spec_Reschedule"){
      let trainer_info=location.state.Trainer;
      let resch_info=location.state.ReschInfo;
      let Resch_Dates=location.state.Resch_Dates;
      dataToPass.push({
        "patientId":trainer_info.patient_id,
        "trainerId":trainer_info.trainer_id,
        "bookingId":trainer_info.trainerBookingId,
        "toBeRescheduleDate":Resch_Dates.toberesh,
        "rescheduledDate":Resch_Dates.rech_date,
        "fromTime":trainer_info.from_time,
        "toTime":trainer_info.to_time
      })
      setParams(dataToPass)
      console.log(dataToPass, "params")
    }
  },[location])
